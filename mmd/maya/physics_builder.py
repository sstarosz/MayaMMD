"""
physics_builder.py — rigid body binding layer for PMX models.

**Architecture (Milestone 2 rewrite — C++ node with embedded Bullet):**

mayaBullet's ``bulletSolverShape`` is a *stateful* node: it advances an internal
Bullet world on every evaluation, and Cached Playback's evaluation cache treats
node outputs as pure functions of their inputs — so it never re-steps the
solver, dynamic bodies froze at rest, and the write-back constraints then
locked the skeleton ("lost mesh binding").  That layer is GONE.

The new binding is driven by one native **``mmdPhysicsNode``** (a plain
``MPxNode`` in ``MayaMMD.mll``) that owns a ``btDiscreteDynamicsWorld``:

* ``mmdPhysicsNode`` is time-driven (``time1.outTime -> node.time``) and
  evaluated by the evaluation manager on every time step — the same path as a
  ``parentConstraint``, which is proven to work under Cached Playback.
* The node declares itself **non-cacheable** (``MPxNode::getCacheSetup`` →
  ``MNodeCacheDisablingInfoHelper::setUnsafeNode``), so Cached Playback always
  re-evaluates it every frame.  This is the one thing mayaBullet's built-in
  solver could not do.
* Every PMX rigid body is an element of the node's ``bodies`` compound array
  (rest pose, mass, damping, collider, collision group/mask, kinematic flag);
  every PMX joint is an element of ``joints`` (type, frame, limits, springs).
* ``FOLLOW_BONE`` bodies are **visible polygonal guide meshes** bound to their
  bone via ``parentConstraint`` (DG).  The guide's world matrix (and its
  parent-inverse) feed the node's ``anchorWorldMatrix`` /
  ``anchorParentInverseMatrix`` inputs, so the Bullet world runs in the physics
  group's local space and the kinematic collider tracks the bone every frame.
* ``PHYSICS`` / ``PHYSICS_BONE`` bodies are **dynamic**: the node writes each
  body's solved LOCAL transform to ``outTranslate[i]`` / ``outRotate[i]``,
  which Python connects straight into the guide mesh's translate/rotate.  A
  ``parentConstraint`` (PHYSICS) or ``orientConstraint`` (PHYSICS_BONE) writes
  the solved pose back to the related bone so the skinned mesh deforms.

The Bullet world advances inside the node's ``compute()``; no solver plugin, no
scriptJob, no pairBlend, no external stateful nodes.

Run it by calling :func:`create_physics_from_pmx_data` directly, or pass
``build_physics=True`` to ``build_pmx_scene`` (the binding is stored on
``MayaPmxData.physics_binding``).
"""
...
class PhysicsBinding:
    ...
    Milestone 2 scope: the node + ALL rigid bodies (FOLLOW_BONE kinematic
    guides and dynamic PHYSICS / PHYSICS_BONE bodies), all PMX joints, and the
    per-frame write-back from dynamic bodies to their related bones.
